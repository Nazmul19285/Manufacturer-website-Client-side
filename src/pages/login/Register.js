...
const Register = () => {
    ...
    if (googleError || error || updateError) {
        loginError = <p className='text-red-500 text-sm'>{googleError?.message || error?.message || updateError?.message}</p>
    }
    ...
    return (
        <div>
            ...
            <div className='mt-12 flex justify-center items-center'>
                <div className='w-80 bg-white drop-shadow-xl p-4 rounded-lg'>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="form-control w-full max-w-xs">
                            ...
                            <input type="password" placeholder="Your password" className="input input-bordered w-full max-w-xs"
                                {...register("password", {
                                    ...
                                    minLength: {
                                        value: 8,
                                        message: 'Should be 8 characters or longer'
                                    }
                                })} />
                            <label className="label">
                        ...
                        {loginError}
                        <input className='btn btn-outline btn-info text-black w-full max-w-xs' type="submit" value="SIGN UP" />
                        <p className='text-sm mt-2'>Already registered? <Link className='text-blue-500' to="/login"> Login here</Link></p>
                    </form>
                    <div className="divider">OR</div>
                    <button className="btn w-full btn-outline btn-secondary" onClick={() => signInWithGoogle()}>Continue with Google</button>
                </div>
            </div>
        </div>
    );
